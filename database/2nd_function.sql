...
BEGIN
    ...
    EXECUTE 'CREATE TABLE districts AS
             ...
             WHERE census_data.city IN (SELECT json_array_elements_text(info->''city'') FROM temp_json);';



    EXECUTE 'CREATE TABLE cities AS
    ...
    ALTER TABLE cities ADD PRIMARY KEY (city);
...
SELECT * FROM select_by_json('{"city": ["Prague", "Marseille", "Busan"]}'::json);


SELECT city, centroid FROM cities;
